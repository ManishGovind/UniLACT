            ...
            for episode in tqdm(iter(ds[split]), total=len(ds[split])):
                try:
                    for j, step in enumerate(episode["steps"].as_numpy_iterator()):
                        ...
                        if dataset_name == "berkeley_fanuc_manipulation" or dataset_name == "bridge_dataset":
                    ...
                    print(f"⚠️ Skipping corrupted episode {ep_idx}: {e}")
                    continue  # Skip to next episode
                except Exception as e:
                    print(f"⚠️ Unexpected error in episode {ep_idx}: {e}")
                    continue    
        env.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_dir", default='/data/vidlab_datasets/robotics', type=str, help="Original root dataset directory.")
    parser.add_argument("--output_dir", default='/data/vidlab_datasets/robotics/lmdb_datasets', type=str, help="Output root dataset directory.")
    parser.add_argument("--dataset_name", default='bridge_dataset', type=str, help="Dataset name.")
