...
class MotoGPT(nn.Module):
    def __init__(
        ...
        if self.use_latent_motion_pos_embedding:
            ...
            # if self.multimodal :
            self.embed_latent_motion_pos = nn.Embedding((per_latent_motion_len+2)*sequence_length, hidden_size) # Added for modality specific token
        ...
        if self.latent_motion_pred:
            ...
            #Modality specific motion token 
            self.rgb_latent_query     = nn.Embedding(1, hidden_size)
            self.depth_latent_query   = nn.Embedding(1, hidden_size)
            self.unified_latent_query = nn.Embedding(1, hidden_size)
            self.modality_query = {
                "rgb": self.rgb_latent_query,
                "depth": self.depth_latent_query,
                "unified": self.unified_latent_query
            }
    ...
    def forward(self, 
        ...
        if rgb_feat is not None :
            ...
            obs_embeddings , patch_embeddings= rgb_feat[:, 0, :] , rgb_feat[:, 1:, :]
        else :            
        ...
        # online = torch.cat([obs_embeddings.unsqueeze(1) , patch_embeddings],dim=1)
        # patch_distances = torch.linalg.norm(rgb_feat - online, ord=2,dim=-1).mean()
        # obs_distances = torch.linalg.norm(obs_embeddings_o - obs_embeddings, ord=2, dim=-1).mean()
        
        # print("online obs magnitude:", torch.norm(obs_embeddings))
        # print("offline obs magnitude:",  torch.norm(obs_embeddings_o))
        # print("online patch magnitude:",  torch.norm(patch_embeddings))
        # print("offline patch magnitude:",  torch.norm(patch_embeddings_o))
        
        # print("patch feat distance in motogpt", patch_distances)
        # print("obs feat distance", obs_distances)
        ...
        if self.latent_motion_pred:
            ...
            #Added modality specific learnable token
            # print(modality)
            modality_query = self.modality_query[modality].weight  #(1,h)
            modality_query = modality_query.view(1, 1, 1, self.hidden_size).repeat(batch_size, sequence_length, 1, 1)  # (b, t, 1, h)
            
            latent_motion_embeddings = self.embed_latent_motion(latent_motion_ids) # (b, t, per_latent_motion_len, h)
            act_stacked_inputs = torch.cat((latent_motion_queries, modality_query, latent_motion_embeddings), dim=2)  # (b, t, per_latent_motion_len+2, h)
        ...
        n_tokens = 0
        n_latent_motion_pred_tokens = 1 + 1+ self.per_latent_motion_len # modality specific token
        if self.latent_motion_pred:
    ...
    def greedy_decode(self, act_x, stacked_inputs, stacked_attention_mask, latent_motion_query_token_start_i, n_cond_tokens, parallel=True, buffer_len=10):
        ...
        for j in range(self.per_latent_motion_len):
            ...
            if self.use_latent_motion_pos_embedding:
                cur_latent_motion_pos_embedding = self.embed_latent_motion_pos.weight[(buffer_len-1)*(self.per_latent_motion_len+2) + j+1] # (h,)
