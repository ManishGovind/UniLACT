...
from unilact.src.models.trajectory_gpt2 import GPT2Model, GPT2Config

class UniLACT(nn.Module):
    def __init__(
            self,
            model_lang,
            model_vision,
            ...
            patch_feat_dim,
            lang_feat_dim,
            mask_latent_action_probability=0.0, # 0.0: do not mask, 1.0: mask all, 0.5: mask 50% latent actions
            freeze_lang=True,
            freeze_vision=True,
        ...
        self.mask_latent_action_probability = mask_latent_action_probability
        ...
        # Latent action Positional Embeddings
        ...
        if self.use_latent_action_pos_embedding:
            print(f"use_latent_action_pos_embedding: {self.use_latent_action_pos_embedding}")
            # self.embed_latent_action_pos = nn.Embedding((per_latent_action_len+1)*sequence_length, hidden_size)
            if is_multimodal :
                self.embed_latent_action_pos = nn.Embedding((per_latent_action_len+2)*sequence_length, hidden_size) # Added for modality specific token
            else :
                self.embed_latent_action_pos = nn.Embedding((per_latent_action_len+1)*sequence_length, hidden_size)    
    ...
    def forward(self, 
                ...
                language,
                attention_mask, # (b, t)
                latent_action_ids, # (b, t, per_latent_action_len)
                latent_mask, # (b, t)
                train=True,
        ...
        if self.freeze_vision:
            with torch.no_grad():
                ...
                # print("Actual shapes:", obs_embeddings.shape , patch_embeddings.shape )
        else:
            obs_embeddings, patch_embeddings = self.model_vision(rgb.reshape(batch_size, c, h, w))  # (b, img_feat_dim), (b, n_patches, patch_feat_dim)
        ...
        if self.latent_action_pred:
            ...
            latent_action_queries = latent_action_queries.view(1, 1, 1, self.hidden_size).repeat(batch_size, sequence_length, 1, 1)  # (b, t, 1, h)

            latent_action_embeddings = self.embed_latent_action(latent_action_ids) # (b, t, per_latent_action_len, h)
            
            act_stacked_inputs = torch.cat((latent_action_queries, latent_action_embeddings), dim=2)  # (b, t, per_latent_action_len+2, h)
                    

            if self.use_latent_action_pos_embedding:
                latent_action_pos_embeddings = self.embed_latent_action_pos.weight # (per_latent_action_len+1, h)
                latent_action_pos_embeddings = latent_action_pos_embeddings.reshape(sequence_length, -1, latent_action_pos_embeddings.shape[-1])
        ...
        n_tokens = 0
        n_latent_action_pred_tokens = 1 + self.per_latent_action_len # modality specific token
        if modality is not None :
            n_latent_action_pred_tokens = 1 + n_latent_action_pred_tokens
            
        if self.latent_action_pred:
            latent_action_query_token_start_i = n_tokens
            n_tokens += n_latent_action_pred_tokens
        ...
        if self.latent_action_pred:
            act_stacked_attention_mask[:,:,latent_action_query_token_start_i:latent_action_query_token_start_i+n_latent_action_pred_tokens] = \
                latent_mask.clone().view(batch_size, sequence_length, 1).repeat(1, 1, n_latent_action_pred_tokens) # (b, t, n_latent_action_pred_tokens)

        if self.act_pred:
            act_stacked_attention_mask[:,:,act_query_token_start_i:act_query_token_start_i+self.chunk_size] = 0 # (b, t, n_act_pred_tokens)

        if not (self.latent_action_pred and self.act_pred):
            ...
            # Randomly mask attention from action query tokens to latent action tokens
            ...
            latent_action_query_attention_mask = latent_mask.clone().view(batch_size, sequence_length, 1)
            probability_matrix = torch.full(latent_action_query_attention_mask.shape, 1-self.mask_latent_action_probability) 
            masked_indices = torch.bernoulli(probability_matrix).bool()
            latent_action_query_attention_mask[~masked_indices] = 0
            latent_action_query_attention_mask = latent_action_query_attention_mask.repeat(1, 1, n_latent_action_pred_tokens) # (b, t, n_latent_action_pred_tokens)
            latent_action_query_attention_mask = latent_action_query_attention_mask.reshape(batch_size, 1, 1, 1, sequence_length, n_latent_action_pred_tokens)\
                .repeat(1, 1, sequence_length, self.chunk_size, 1, 1) # (b, 1, t, n_act_pred_tokens, t, n_latent_action_pred_tokens)
            act_stacked_attention_mask[:, :, :, act_query_token_start_i:act_query_token_start_i+self.chunk_size, \
                :, latent_action_query_token_start_i:latent_action_query_token_start_i+n_latent_action_pred_tokens] = latent_action_query_attention_mask
        ...
        if self.latent_action_pred:
            if train:
                latent_action_x = act_x[:, :, latent_action_query_token_start_i:(latent_action_query_token_start_i+self.per_latent_action_len)] # (b, t, per_latent_action_len, h)
                latent_action_preds = self.pred_latent_action_head(latent_action_x) # (b, t, per_latent_action_len, latent_action_codebook_size)
            else:
                in_simulation = kwargs.pop("in_simulation", False)
                if in_simulation and self.act_pred and (self.mask_latent_action_probability == 1.0):
                    # Skip latent action prediction while evaluating Moto-GPT in simulation environments
                    latent_action_id_preds = torch.zeros(batch_size, self.per_latent_action_len).long()
                else:
        ...
        prediction = {
            ...
            'latent_action_preds': latent_action_preds
        }
    ...
    def greedy_decode(self, act_x, stacked_inputs, stacked_attention_mask, latent_action_query_token_start_i, n_cond_tokens, parallel=True, buffer_len=10):
        ...
        for j in range(self.per_latent_action_len):
            ...
            if self.use_latent_action_pos_embedding:
                cur_latent_action_pos_embedding = self.embed_latent_action_pos.weight[(buffer_len-1)*(self.per_latent_action_len+1) + j+1] # (h,)
