...
"""Class for evaluating UniLACT on Robot Manipulation Benchmarks."""
import torch
...
class UniLACT_PolicyWraper:
    def __init__(self,
        ...
        self.mask_latent_action_probability = variant['mask_latent_action_probability']
        self.latent_action_pred = variant['latent_action_pred']
        self.per_latent_action_len = variant['per_latent_action_len']

        assert self.mask_latent_action_probability == 0.0 or self.mask_latent_action_probability == 1.0
        self.test_seq_len = math.ceil(self.test_chunk_size / self.chunk_size)
        self.latent_action_decoding_kwargs = latent_action_decoding_kwargs
    ...
    def depth_process(self, depth):
        ...
        depth = T.ToTensor()(depth.convert('RGB'))
    ...
    def step(self, obs, goal):
        ...
        #Depth
        # depth = self.depth_process(obs['depth_obs']['depth_static'])
        # depth_data = depth.unsqueeze(0).unsqueeze(0)
        
        # gripper_rgb = self.rgb_process(obs['rgb_obs']['rgb_gripper'])
        # gripper_rgb_data = gripper_rgb.unsqueeze(0).unsqueeze(0)
        
        
        # Latent action tokens
        if self.latent_action_pred:
            latent_action_ids_data = torch.zeros((1, self.seq_len, self.per_latent_action_len)).long() # (1, t, per_latent_action_len)
        else:
            latent_action_ids_data = None
        ...
        latent_action_ids_data = latent_action_ids_data.to(self.device) if self.latent_action_pred else None
        latent_mask = latent_mask.to(self.device) if self.latent_action_pred else None

        latent_action_decoding_kwargs = self.latent_action_decoding_kwargs
        latent_step = 1
        latent_action_decoding_kwargs['buffer_len'] = latent_step
        with torch.no_grad():
            ...
            prediction = self.policy(
                    ...
                    latent_action_ids=latent_action_ids_data,
        ...
        if self.mask_latent_action_probability == 0.0:
            latent_action_ids_data[:, latent_step-1] = prediction['latent_action_id_preds'].to(self.device)
            # print(latent_action_ids_data[:, latent_step-1])
