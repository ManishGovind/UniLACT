...
class LMDViTEmbeddings(nn.Module):
    ...
    def forward(
        ...
        latent_action_tokens: torch.Tensor,
    ) -> torch.Tensor:
        ...
        if self.query_fusion_mode == 'add':
            # add the projected action token to the embedded patch tokens
            latent_action_tokens = latent_action_tokens.reshape(batch_size, 1, -1)
            latent_action_tokens = self.query_pooling_layer(latent_action_tokens)
            embeddings = embeddings + latent_action_tokens
        elif self.query_fusion_mode == 'concat':
            # concatenate latent action tokens with the embedded patch tokens
            embeddings = torch.cat((latent_action_tokens, embeddings), dim=1)
...
class LMDViTModel(ViTPreTrainedModel):
    ...
    def forward(
        ...
        latent_action_tokens: Optional[torch.Tensor] = None, # change lam_tokens to latent_action_tokens
