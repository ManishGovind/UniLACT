...
from unilarn.src.trainers.trainer_utils import visualize_latent_motion_reconstruction
import omegaconf
from glob import glob
import shutil
from collections import defaultdict
from unilarn.src.trainers.optimizer import get_optimizer, LinearWarmup_CosineAnnealing
from contextlib import contextmanager
import wandb
from tqdm import tqdm

class UniLARN_Trainer:
    def __init__(
        self,
        unilarn,
        rgb_preprocessor,
        ...
        if resume_ckpt_path is not None:
            print(f"resuming UniLARN from {resume_ckpt_path} ...")
            missing_keys, unexpected_keys = unilarn.load_state_dict(torch.load(os.path.join(resume_ckpt_path, 'pytorch_model.bin'), map_location='cpu'), strict=False)
        ...
        optimizer = get_optimizer(
                        [p for n, p in unilarn.named_parameters() if p.requires_grad], 
                        lr=lr_max, 
                        wd=weight_decay
                    )
        ...
        unilarn, optimizer, train_dataloader, eval_dataloader = accelerator.prepare(
            unilarn, optimizer, train_dataloader, eval_dataloader, 
    ...
    def save_checkpoint(self, save_dir):
        unwrapped_unilarn = self.accelerator.unwrap_model(self.unilarn)
        state_dict = unwrapped_unilarn.get_state_dict_to_save()
        
        torch.save(state_dict, os.path.join(save_dir, "pytorch_model.bin"))
        omegaconf.OmegaConf.save(unwrapped_unilarn.config, os.path.join(save_dir, "config.yaml"))
    ...
    def train(self):
        ...
        with tqdm(total=self.num_epochs, leave=False) as progress:
            for epoch in range(self.num_epochs+1):
                ...
                clock = time()
                batch_idx = 0
                progress.set_description("Epoch " + str(epoch+1))
                
                batch, load_time = self.train_prefetcher.next()

                while batch is not None:
                    with self.accelerator.accumulate(self.unilarn):
    ...
    def eval_latent_motion_reconstruction(self, visualization_dir):
        ...
        if "recons_depth_values" in outputs :
            recons_depth_future = self.rgb_preprocessor.post_process(outputs["recons_depth_values"]).detach().cpu()
                  
        gt_latent_action_ids = outputs["indices"].detach().cpu() # (b, per_latent_action_len)
        orig_rgb_seq = self.rgb_preprocessor.post_process(rgb_seq).detach().cpu()
        if use_depth :
            orig_depth_seq =  self.rgb_preprocessor.post_process(depth_seq, depth=True).detach()
        for i in range(orig_rgb_seq.shape[0]):
            visualize_latent_motion_reconstruction(
                ...
                latent_action_ids=gt_latent_action_ids[i],
